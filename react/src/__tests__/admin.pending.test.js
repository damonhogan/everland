...
// dynamic import of get-port (fallback to fixed port if unavailable)
...
async function startServer() {
  try {
    const gpMod = await import('get-port')
    const gp = gpMod.default || gpMod
    port = await gp({ port: gp.makeRange(3001, 3010) })
  } catch (e) {
    port = 3040
  }
  serverProc = spawn('node', ['server.js'], { cwd: root, env: { ...process.env, PORT: String(port) }, stdio: ['ignore', 'pipe', 'pipe'] })
...
test('admin pending list and approve flow', async () => {
  const pendingPath = path.join(root, 'public', 'bbs', 'pending_assets.json')
  ...
  const audioPath = path.join(root, 'public', 'bbs', 'audio_assets.json')
  ...
  const npcsPath = path.join(root, 'public', 'bbs', 'npcs_cards.json')
  ...
  const scenesPath = path.join(root, 'public', 'bbs', 'lore_scenes.json')
